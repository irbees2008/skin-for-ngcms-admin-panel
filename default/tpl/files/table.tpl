...
<div class="content tabs clear">
	...
	<div class="div-resp">
		...
		<form action="{php_self}?mod=files" method="post" enctype="multipart/form-data" name="sn">
			...
			<div class="input-group">
				<span class="input-group-check"><input type="checkbox" name="replace" value="replace" id="flagReplace" value="1"/></span>
				<input type="text" value="{l_do_replace}" disabled />
			</div>
			<div class="input-group">
				<span class="input-group-check"><input type="checkbox" name="rand" value="rand" id="flagRand" value="1"/></span>
			...
			<span id="showRemoveAddButtoms">
				...
				<input type="button" value='{l_onemore}' onClick="AddFiles();return false;" /><br /><br />
			</span>
			
			<table id="fileup" class="upload">
				<tr id="row">
					<td>1: </td><td><input type="file" id="fileUploadInput" name="userfile[0]" /></td>
				</tr>
			</table>
		...
		<form action="{php_self}?mod=files" method="post" name="snup">
				...
				<tr id="row">
					<td>
						<div class="input-group">
							...
							<input type="text" name="userurl[0]" />
						</div>
					</td>
				</tr>
			</table>
			...
			function AddFiles2() {
				...
				var row = tbl.insertRow(lastRow);
				var cellRight = row.insertCell(0);
				cellRight.innerHTML = '<div class="input-group"><span class="input-group-check">'+ iteration +'</span><input type="text" name="userurl[' + lastRow + ']" /></div>';
			}
